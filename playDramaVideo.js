		...
		getDataSource.getDataSource(keyArray
			...
			, function (data) {
				...
				if (stuPlayTime[0]) {
					...
					//$scope.begintimespan = angular.copy(stuPlayTime[0].studytime);
					$scope.pageindex = $scope.studytime;
					if ($scope.pageindex == 0) {
						$scope.pageindex = 1;
						//$scope.begintimespan = 1;
					}
...
app.controller("questionController", ["$scope", "$rootScope", "getDataSource", "$stateParams", "CommonService", "DateService", function ($scope, $rootScope, getDataSource, $stateParams, CommonService, DateService) {
        ...
        getDataSource.getUrlData("../api/searchQuestions",
            ...
            function (data) {
                ...
                if (myQuestions && myQuestions.length > 0) {
                	$scope.questions = _.union($scope.questions, myQuestions);
                    $scope.currentPageIndex++;
    ...
    $scope.attention = function (id) {
    	...
    	if (!attention.isattention) {
    		...
    		getDataSource.getUrlData("../api/cancerCommonAttention",
				{
					"eventid": id,
					"accountid": $rootScope.user.accountId,
					"passive_classid": attention.classid,
					...
					if (flag == "true") {
						attention.attentioncount--;
						attention.isattention = 0;
					}
				}, function (errortemp) { });
    	}
    }

    $scope.commonPraise = function (like) {
    	getDataSource.getUrlData("../api/commonPraise",
			{
				"eventid": like.id,
				"accountid": $rootScope.user.accountId,
    ...
    $scope.cancerCommonPraise = function (like) {
    	getDataSource.getUrlData("../api/cancerCommonPraise",
			{
				"eventid": like.id,
				"accountid": $rootScope.user.accountId,
				"passive_classid": like.classid,
    ...
    $scope.clickALike = function (id) {
    	...
    	if (!like.isclick) {
    		$scope.commonPraise(like);
    	} else {
    		$scope.cancerCommonPraise(like);
    	}
    }
...
app.controller("videoCouresController", ["$scope", "$rootScope", "$anchorScroll", "$location", "$document", "$stateParams", "$modal", "getDataSource", "FilesService", "CommonService", "DateService", function ($scope, $rootScope, $anchorScroll, $location, $document, $stateParams, $modal, getDataSource, FilesService, CommonService, DateService) {
    ...
    getDataSource.getDataSource(["videoCourse-coursewareComment",
        ...
        if (data && data.length > 0) {
            ...
            if ($scope.avgscore == "0.0") {
                $scope.isavgscoreshow = true;
                $scope.avgscore = "暂无评分";
            }
        ...
        var modalInstance = $modal.open({
            ...
            resolve: {
                ...
                realduration: function () {
                    return $scope.coursewareCommentData.realduration;
                }
            }
        });
    };
...
//提问
app.controller('questionCtrl', ["$rootScope", "$scope", "$modalInstance", "$stateParams", "getDataSource", "CommonService", "videoPlay", "videoTime", "realduration", function ($rootScope, $scope, $modalInstance, $stateParams, getDataSource, CommonService, videoPlay, videoTime, realduration) {
    ...
    $scope.saveQuestion = function () {
        ...
        var faq_id = getDataSource.getGUID();
        //判断视频还是pdf
        getDataSource.getDataSource("getVideoTypesql", {
            coursewareid11: $stateParams.coursewareid,
        }, function (data) {
            if (data[0].videotype != 3) {
                getDataSource.getDataSource("insert_sy_class_faq",
                    {
                        id: faq_id,
                        accountid: $rootScope.user.accountId,
...
//笔记
app.controller('studynoteCtrl', ["$rootScope", "$scope", "$modalInstance", "$stateParams", "getDataSource", "DateService", "CommonService", "videoPlay", "videoTime", "coursewareCommentData", "studynoteConfig", "realduration", function ($rootScope, $scope, $modalInstance, $stateParams, getDataSource, DateService, CommonService, videoPlay, videoTime, coursewareCommentData, studynoteConfig, realduration) {
    ...
    $scope.saveStudynote = function () {
        ...
        getDataSource.getDataSource("getVideoTypesql", {
            ...
            if (data[0].videotype != 3) {
                ...
                getDataSource.getDataSource("insert_sy_classcourse_PDFnote",
                     {
                         id: id,
                         coursewareid: $stateParams.coursewareid,
                         videostamp: realduration,
